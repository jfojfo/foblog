...
class IPInfo(object):
    ...
    def getIPAddr(self, ip):
        ...
        tmp = ip.split(".")
        #tmp2 = "%c%c%c%c" % (int(tmp[0]), int(tmp[1]), int(tmp[2]), int(tmp[3]))
        tmp2 = pack("BBBB", int(tmp[0]), int(tmp[1]), int(tmp[2]), int(tmp[3]))
        ip = unpack('!I', tmp2)[0]
    ...
    def output(self, first, last):
        for i in range(first, last):
            ...
            tmp = pack('!I', unpack('I', self.img[o:o+4])[0])
            tmp2 = unpack('BBBB', tmp)
            ip = "%d.%d.%d.%d" % (tmp2[0], tmp2[1], tmp2[2], tmp2[3])
